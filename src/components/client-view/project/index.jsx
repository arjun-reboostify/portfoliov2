                                    ...
                                    <div className="border-2 w-full relative border-green-main transition-all rounded-lg flex flex-col h-full">
                                        <div className="flex p-3 sm:p-4 flex-col w-full items-stretch">
                                            <div className="flex">
                                                <div className="flex flex-col">
                                                    <h3 className="text-xl sm:text-2xl md:text-3xl text-black-600 capitalize font-bold">{item.name}</h3>
                                                    <p className="text-xs sm:text-sm mt-1 sm:mt-2 text-black-500 capitalize font-bold">{item.createdAt.split("T")[0]}</p>
                                                    ...
                                                    <div className="grid gap-1 sm:gap-2 mt-3 sm:mt-5 grid-cols-2 h-full pb-12">
                                                        {item?.technologies.split(",").map((techItem, techIndex) => (
                                                            <div key={techIndex} className="w-full flex justify-start items-center">
                                                                <button className="whitespace-nowrap text-ellipsis overflow-hidden py-1 sm:py-2 w-[100px] sm:w-[120px] px-2 sm:px-4 md:px-6 border-[2px] border-green-main bg-[#fff] text-[#000] font-semibold rounded-lg text-[10px] sm:text-xs tracking-widest hover:shadow-green-main transition-all outline-none">
                                                                    {techItem}
                                                                </button>
                                                            </div>
                                                        ))}
                                        ...
                                        <div className="absolute w-full bottom-0 justify-center flex gap-2 p-2">
                                            <button 
                                                ...
                                                className="py-1 sm:py-2 px-2 sm:px-3 text-white-300 font-semibold text-[12px] sm:text-[14px] tracking-widest bg-green-main transition-all outline-none hover:bg-green-600"
                                            >
                                                Website
                                            </button>
                                            <button  
